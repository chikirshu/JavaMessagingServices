	...
	public static void main(String[] args) throws NamingException, InterruptedException, JMSException {
		...
		try (ActiveMQConnectionFactory cf = new ActiveMQConnectionFactory();
			...
			TextMessage textMessage = jmsContext
					.createTextMessage("Arise, awake and not stop until the goal is reached.");
			textMessage.setBooleanProperty("loggedIn", true);
			textMessage.setStringProperty("userToken", "abc123");
			producer.send(queue, textMessage);

			Message messageReceived = jmsContext.createConsumer(queue).receive(5000);// wait for 5 seconds to get the
